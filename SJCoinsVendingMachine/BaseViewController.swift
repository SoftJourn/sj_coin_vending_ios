...
class BaseViewController: UIViewController {
    ...
    func fetchProducts() {
        ...
        DownloadManager.fetchListOfProducts { object, error in
            guard let object = object else { return self.handle(error) }
            DataManager.singleton.save(object)
            updateProducts()
        }
    }
    ...
    func fetchFavorites() {
        ...
        DownloadManager.fetchFavorites { object, error in
            guard let object = object else { return self.handle(error) }
            DataManager.singleton.save(object)
            updateFavorites()
        }
    ...
    func fetchAccount() {
        ...
        DownloadManager.fetchAccount { object, error in
            guard let object = object else { return self.handle(error) }
            DataManager.singleton.save(object)
            updateAccount()
        }
